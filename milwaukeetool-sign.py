...
def process_account(account_info, index, total, failed_list):
    token = os.getenv('MILWAUKETOOL_TOKEN_LIST', '').split(',')[index-1].strip()
    client_id = os.getenv('MILWAUKETOOL_CLIENT_ID', '').split(',')[index-1].strip()
...
def main():
    ...
    print("=" * 60)
    success_count = 0
    failed_list = []  # 存储 (账号索引, 原因)
    # 双账号执行（仅开启循环，完全保留原process_account调用方式）
    total_account = 2
    for i in range(1, total_account+1):
        print(f"\n👉 开始处理第{i}个账号/{total_account}个")
        if process_account(0, i, total_account, failed_list):
            success_count += 1
        # 账号间加延时，避免请求频繁
        if i < total_account:
