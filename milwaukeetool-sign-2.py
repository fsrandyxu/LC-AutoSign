...
def format_sign_status(json_data):
    ...
    try:
        ...
        if data.get('status') != 200:
            return f"❌ 错误：API 回应异常 (状态码: {data.get('status')})"
...
def process_account(account_info, index, total, failed_list):
    token = os.getenv('MILWAUKEETOOL_TOKEN_LIST_2', '')
    client_id = os.getenv('MILWAUKEETOOL_CLIENT_ID_2', '')
